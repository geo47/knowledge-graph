...
class StanfordNER:
    ...
    def get_stanford_ner_location(self):
        ...
        # if(loc == ''):
        loc = "./libs/stanford-ner-2018-10-16"
        ...
        for tree in tree_list:
            for st in tree:
                ...
                if isinstance(st, nltk.Tree):
                    ...
                    # else:
                    #     tree_dict[st.label()] = ' '.join([tup[0] for tup in st]) #st[0][1]]
        pprint(tree_dict)
        return tree_dict
...
class CoreferenceResolver:
    ...
    def generate_coreferences(self, doc, stanford_core_nlp_path, verbose):
        ...
        dict of dict of lists of dicts:  { { [ {} ] } }  -- We are interested in the 'corefs' key { [ { } ] }
        -- Each list has all coreferences to a given pronoun.

        dict -> sentence
        ||
        dict -> index
        ||
        list -> dict -> token_schema
        '''
    ...
    def resolve_coreferences(self, corefs, doc, ner, verbose):
        ...
        if verbose:
            for key, val in sentence_wise_replacements.items():
                print("Sent no# ", key)
                for item in val:
                    print(item[0]["text"], " ", item[0]["startIndex"], " ", item[0]["endIndex"], " -> ",
                          replace_coref_with[item[1]], " replacement correl #", item[1], end="   ||| ")
                print("\n")
        ...
        for index, sent in enumerate(sentences):
            ...
            for item in replacement_list[::-1]:                     # item : ({},int)
                ...
                if verbose:
                    print("Original: ", sent)
                    print("To replace:", to_replace["text"], " | at:", to_replace["startIndex"],
                          to_replace["endIndex"], end='')
...
def main():
    ...
    parser.add_argument('--ner_type', type=str, help="Possible values (spacy|nltk|stanford) "
                                                     "Used only if --use_ner is true")
    parser.add_argument('--coref_resolve', action='store_true', default=False, help="Resolve Co-reference")
    parser.add_argument('--core_nlp_path', type=str,
                        default="/home/muzamil/Projects/Python/ML/NLP/KG/knowledge-graph/stanford-corenlp-4.2.2",
                        help="Path of Core NLP library, required when --coref_resolve is true")
    parser.add_argument('--verbose', action='store_true', default=True, help="Log")

    args = parser.parse_args()

    verbose = args.verbose
    execute_coref_resolve = args.coref_resolve
    stanford_core_nlp_path = args.core_nlp_path


    print(verbose, execute_coref_resolve, stanford_core_nlp_path)
    ...
    stanford_core_nlp_path = './libs/stanford-corenlp-4.2.2'
    ...
    for file in file_list:
        ...
        if args.use_ner:
            if args.ner_type == "":
                print("--ner_type required for NER")
                return None
            ...
            if args.ner_type == "nltk":
                ...
                # ner_to_dict for nltk_ner implemented but not using as results are not good enough as spacy
