...
def getAllObjs(v):
    ...
    if compounds:
        objs.append(compounds)
    # for obj in objs:
    #     compounds = get_compounds(obj)
    #     if compounds:
    #         objs.remove(obj)
    #         objs.extend([tok for tok in compounds])
            # compound_obj = compounds
            # compound_obj.append(str(obj))
            # objs.insert(obj, compound_obj)
    objs.extend(getObjsFromPrepositions(rights))
...
def printDeps(toks):
    for tok in toks:
        sub_part = [t.orth_ for t in tok.lefts]
        obj_part = [t.orth_ for t in tok.rights]

        if len(sub_part) > 0 and len(obj_part) > 0:
            print(sub_part, tok.orth_, obj_part)
...
def testSVOs():
    ...
    # tok = nlp("Angela visit silver spoon restaurant with Angela friends")
    # tok = nlp("silver spoon restaurant food quality is amazing")
    # tok = nlp("silver spoon restaurant food as well as services are all good")
